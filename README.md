...
The persistence slice proves durable idempotent single-file publishing, restart recovery, multipart object upload, and byte-range delivery. The authentication foundation uses Better Auth for closed-registration owner sessions and Shelf-owned, workspace-scoped agent credentials with rotation, revocation, and audit history. A production server and host-local operator CLI now make that path runnable through the single-host Docker Compose alpha. The operator can also perform an age-gated, read-only reconciliation scan across PostgreSQL and either content adapter. Backup/restore drills, destructive cleanup policy, administrative password recovery, and live R2 conformance remain intentionally incomplete.
