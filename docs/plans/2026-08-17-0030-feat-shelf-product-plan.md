...
3. **Artifact lifecycle breadth:** Stable file updates plus artifact list/detail/history are implemented first; next add rename and restore-as-latest, then atomic folder snapshots with portable path rules, version comparison, and the first size/file-count limits.
...
| OpenAPI drift check | `/api/v1` | The generated specification matches the checked contract artifact and includes artifact create/update/catalog/history plus pinned revision delivery. |
...
- U1-U14 satisfy their applicable test scenarios and verification outcomes.
