...
- KTD17. **Build one dark web client and a separate active-content renderer.** (session-settled: user-directed — chosen over a themeable dashboard and same-origin preview: Shelf should stay minimal and dark while authored active content cannot share the authenticated trust context.) Use React Router Data Mode, Tailwind CSS 4 for product layout, Cloudflare Kumo's managed Base UI components for generic controls, Phosphor icons, Geist Sans/Mono, and reduced-motion-aware package/CSS interaction in `apps/web`. Shelf owns the artifact map bar, viewer, responsive inspector, semantic tokens, and renderer boundary rather than hand-building every button, field, dialog, menu, table, toast, or clipboard state. Do not add a second component foundation. A specialist package may enter only for one deep accepted behavior behind a replaceable adapter; `@pierre/trees` requires a focused beta qualification spike and `@pierre/diffs` waits for content-aware comparison. The first passive allowlist is escaped UTF-8 text/source/JSON, sanitized Markdown without raw HTML, raster images, and folder trees. Self-contained HTML runs only through `apps/renderer` on a separately configured hostname/origin, with no application authentication secret or session cookie and no same-origin, form, popup, download, or top-navigation privileges. The application rejects a renderer on its own hostname, and the renderer rejects cookie-bearing render requests. CSP blocks fetch, XHR, and subresource egress. A private runtime completion channel lets the parent reject a parse-time replacement and blank every later iframe navigation; Chromium may still issue one own-frame navigation request before termination, so strict zero-egress installations keep HTML download-only. SVG, PDF, other active media, and unknown binaries remain downloads. The [Shelf Interface Standard](../design/shelf-interface-standard.md) governs visual composition and qualification. Governs R7, R14-R15, R21, AE5, and implements accepted T6/T8.
...
- KTD19. **Model Protected and Public as access policies on one Share lifecycle.** (session-settled: user-approved — chosen over separate share systems.) Protected retains the reusable fragment capability, optionally expires, and may count successful viewer-session establishments; a signed tab-scoped token renews the same session without another use. Public uses a globally unique 12-character CSPRNG selector, credential-free anonymous reads, mandatory expiry within 30 days, and a 24-hour default. Both support Latest or pinned targets, remain unlisted and non-indexed, expose server-projected lifecycle state, and share the same authenticated create/list/revoke contract. Public is explicitly non-confidential and does not add discovery, aliases, analytics, passwords, or permanent links. Implements accepted T11 and governs R10-R17, R21, R26, F3, AE1, AE3, and AE5.
...
- **Approach:** Add provider-neutral share contracts, a core share lifecycle, PostgreSQL persistence, authenticated idempotent create/list/revoke operations, explicit Protected and Public anonymous routes, and portable `shelf shares` commands. Protected links use KTD16's fragment capability once to establish KTD19's signed viewer session; Public links use KTD19's short expiring selector without a secret. Shares target one artifact's latest revision or one exact revision and never group artifacts. Keep new artifacts private and make share creation an explicit action. Build the first `apps/web` route as a minimal dark trust boundary around the artifact: a slim title rail, target state, an explicit user-generated-content warning, and content occupying the remaining viewport. Apply KTD17's passive allowlist; self-contained HTML runs only through the isolated renderer origin and unsupported content remains a safe download.
- **Execution note:** Protected URLs contain capability-bearing secrets and must not enter request logs, error details, referrers, or browser history unintentionally. Public selectors are non-confidential but remain unlisted, non-indexed, expiring, and revocable. A share changes access, not artifact retention. This unit does not add CLI profiles, implicit sharing, collections, a dashboard application, password protection, analytics, aliases, or public discovery.
- **Implementation status:** Protected/Public creation, persistence, lifecycle projection, viewer-session establishment, secretless Public reads, renderer authority, reusable management URLs, dashboard controls, CLI parity, and generated OpenAPI are implemented. Public defaults to 24 hours and is capped at 30 days; Protected defaults to no expiry and unlimited sessions. Real PostgreSQL concurrency coverage is present but requires `SHELF_TEST_POSTGRES_URL` and was not executed in the current environment.
- **Test scenarios:**
...
- **Goal:** Make the common installed workflow `shelf publish ./path --title <title> --description <description> [--share]` after one explicit profile setup, while retaining canonical JSON and safe retry behavior for agents.
...
- **Approach:** Add named profiles that select one installation, credential, and default workspace without mixing contexts. Accept a positional file or folder path and dispatch to the existing separate transports. Require agent-oriented positional publishes to provide title and description metadata while retaining arbitrary repeatable metadata; `--user-bypass` is an explicit human escape hatch, not a validation bypass. Keep `--share` an explicit opt-in that creates an unlisted latest Protected share by default and returns artifact, revision, and share URLs; policy flags may instead select Public, expiry, and Protected session limits. Preserve the complete explicit legacy context for automation and backwards compatibility, but never mix its authority fields with a profile-backed invocation. Define replay and partial-failure semantics before implementation so a committed artifact is never reported as absent when later share creation fails.
...
- **Implementation status:** Profile-backed file/folder dispatch, local retry journal, metadata shorthand and enforcement, Protected/Public share flags, access-aware URL validation, redacted JSON output, and comprehensive command help are implemented.
- **Test scenarios:**
...
- **Implementation status:** The dark Artifacts/Access utility, human-session APIs, reveal-once credential administration, bounded artifact/history/share/folder pagination, lifecycle dialogs, comparison, and isolated HTML handoff are implemented. Focused contract/auth/API/web tests, generated OpenAPI, production builds, and interactive Chromium checks at 1440 px, 320 px, and a 720 px 200%-layout equivalent pass with no axe violations or horizontal overflow. The pinned Playwright harness collects 25 desktop/mobile, reduced-motion, axe, capability, and renderer-escape scenarios across Chromium, WebKit, and Firefox; all 20 Chromium/WebKit scenarios pass. Cross-engine qualification exposed and fixed a same-hostname cookie boundary in the local renderer profile: Shelf and its renderer now require distinct hostnames, and cookie-bearing render requests fail before capability resolution. Firefox remains unqualified because its pinned browser build cannot initialize on the current macOS 27 host before any Shelf page loads. True native 200% browser zoom also remains distinct from the passing 720 px layout-equivalent check.
- **Test scenarios:**
