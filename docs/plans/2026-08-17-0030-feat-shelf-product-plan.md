...
3. **Artifact lifecycle breadth:** Stable file updates, artifact list/detail/history, mutable naming, restore-as-latest, atomic folder snapshots, portable path rules, and the first folder limits are implemented first; next add version comparison.
...
| OpenAPI drift check | `/api/v1` | The generated specification matches the checked contract artifact and includes file and folder create/update/catalog/history/tree/rename/restore plus pinned file revision delivery. |
...
- U1-U16 satisfy their applicable test scenarios and verification outcomes.
