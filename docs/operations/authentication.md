...
The server distribution ships a separate `shelf-admin owner bootstrap` command and an in-container `shelf` prompt for owner setup and recovery. Both accept a password only through a protected file or standard input, never a password argument, log, or committed environment file. `shelf-admin owner reset` updates the existing Better Auth identity and Shelf actor name in one PostgreSQL transaction, preserves the actor ID and grants, and revokes every existing browser session.
...
The portable `shelf` CLI remains a remote artifact client, while `shelf-admin` remains the underlying host-local recovery and bootstrap boundary. The web-server container does not install the portable client; its lightweight `/usr/local/bin/shelf` command is an interactive owner-administration wrapper only. Social login, passkeys, external OIDC, teams, invitations, Better Auth Organizations, and Better Auth machine-token plugins remain deferred.
